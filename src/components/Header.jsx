...
function Header() {
  ...
  return (
    <section className="bg-white sticky top-0 z-50">
        ...
        <div className="flex gap-6 md:gap-10 items-center">
          ...
          <div className="hidden md:flex gap-5">
            <NavLink
              to="/help"
              className={({ isActive }) => 
                `text-sm lg:text-base hover:text-primary transition-colors ${isActive ? "text-primary font-medium" : "text-gray-700"}`
              }
            >
              Help
            </NavLink>
      ...
      {/* Mobile Menu */}
      <div className={`w-full bg-white shadow-md md:hidden transition-all duration-300 ease-in-out border-t border-gray-200 overflow-hidden ${isOpen ? 'max-h-96 py-4' : 'max-h-0 py-0'}`}>
        <div className="flex flex-col items-center gap-6 px-4">
          <NavLink
            to="/help"
            className={({ isActive }) => 
              `w-full text-center py-2 text-gray-700 hover:text-primary transition-colors ${isActive ? "text-primary font-medium" : ""}`
            }
            onClick={() => setIsOpen(false)}
          >
            Help
